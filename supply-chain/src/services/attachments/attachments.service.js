...
export async function listChainAttachments(orderId) {
  const c = getClient();
  const [{ data: dns }, { data: sis }, { data: grs }] = await Promise.all([
    ...
    c.from('goods_receipts').select('id,grn_number').eq('order_id', orderId),
  ]);
  const dnIds = (dns || []).map((d) => d.id), siIds = (sis || []).map((s) => s.id), grIds = (grs || []).map((g) => g.id);
  const dnName = {}; (dns || []).forEach((d) => { dnName[d.id] = d.dn_number; });
  const siName = {}; (sis || []).forEach((s) => { siName[s.id] = s.invoice_number; });
  const grName = {}; (grs || []).forEach((g) => { grName[g.id] = g.grn_number; });
  ...
  if (grIds.length) qs.push(c.from('document_attachments').select(sel).eq('doc_type', 'goods_receipt').in('doc_id', grIds).eq('superseded', false));
  ...
  results.forEach((r) => (r.data || []).forEach((a) => out.push({
    ...
    doc_label: a.doc_type === 'purchase_order' ? 'Purchase Order'
      ...
      : a.doc_type === 'goods_receipt' ? 'Receipt ' + (grName[a.doc_id] || '#' + a.doc_id)
