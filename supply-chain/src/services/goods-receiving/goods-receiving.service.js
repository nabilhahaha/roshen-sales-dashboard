...
// inventory_movements (the source of truth) — the inventory table is a derived
// running balance. Below-minimum shelf-life accept/reject decisions are written
// to the audit log.
...
// Release the GR: post inventory movements for every Released batch, upsert the
// running balance, and set header/DN status. costByKey optional (unit cost).
export async function releaseGoodsReceipt(grId, opts = {}) {
  ...
  for (const b of released) {
    ...
    if (em) throw em;
    // 2) running balance upsert
    const { data: existing } = await c.from('inventory')
      .select('id,cases_on_hand').eq('roshen_id', b.roshen_id || '')
      .eq('batch_no', b.batch_no || '').eq('expiry_date', b.expiry_date).eq('warehouse', warehouse || '').limit(1);
    if (existing && existing.length) {
      await c.from('inventory').update({
        cases_on_hand: Number(existing[0].cases_on_hand || 0) + Number(b.received_cases || 0),
      ...
      await c.from('inventory').insert({
        item_code: b.item_code, roshen_id: b.roshen_id, description: b.description,
        batch_no: b.batch_no, manufacturing_date: b.manufacturing_date, expiry_date: b.expiry_date,
        warehouse, cases_on_hand: Number(b.received_cases || 0),
      });
    }
    // 3) mark DN batch released
