// Order History — list purchase orders with lifecycle actions.
...
export async function render(root, ctx) {
  ...
  async function load() {
    ...
    try { ORDERS = await listOrders(); } catch (e) { body.innerHTML = emptyState('⚠', e.message || String(e)); return; }
    paint(ORDERS);
  }
  function paint(list) {
    if (!list.length) { body.innerHTML = emptyState('📭', 'No purchase orders yet. Click “New Order” to create one.'); return; }
