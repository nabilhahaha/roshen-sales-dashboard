...
export async function render(root, ctx) {
  ...
  function exportLinesExcel(o) {
    ...
    const rows = lineData(o), t = lineTotals(rows);
    const header = ['Roshen Code', 'ERP Item Code', 'Description', 'Ordered Qty', 'Shipped Qty',
      'Delivered Qty', 'Remaining Qty', 'Line Status', 'Purchase Order Number', 'Supplier'];
    ...
    aoa.push([], ['TOTAL', '', '', t.ordered, t.shipped, t.delivered, t.remaining, '', '', ''],
  ...
  function openLinesPrintDoc(o, { pdf }) {
    ...
    w.document.write(`<!doctype html><html><head><title>${esc(o.order_number)} — Open Order Items</title><style>
      ...
      <table><thead><tr><th>Roshen Code</th><th>ERP Item Code</th><th>Description</th>
        <th class="r">Ordered</th><th class="r">Shipped</th><th class="r">Delivered</th><th class="r">Remaining</th><th>Line Status</th></tr></thead>
      <tbody>${rows.map((r) => `<tr>${cells(r)}</tr>`).join('')}</tbody>
      <tfoot><tr><td colspan="3">TOTAL — ${rows.length} line(s)</td>
        <td class="r">${qty(t.ordered)}</td><td class="r">${qty(t.shipped)}</td><td class="r">${qty(t.delivered)}</td>
        <td class="r">${qty(t.remaining)}</td><td></td></tr></tfoot></table>
    ...
    const rows = orders.map((o) => {
      const lines = OPEN_LINES.has(o.id) ? `<tr><td colspan="11" style="background:var(--bg-secondary);padding:8px 14px">
        <div class="oo-lines-bar">
          <b style="font-size:12.5px">📦 Item Details — ${esc(o.order_number)}</b>
